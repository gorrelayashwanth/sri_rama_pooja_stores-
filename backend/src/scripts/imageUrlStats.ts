...
async function main() {
  ...
  for (const { url } of images) {
    if (url.includes('cloudinary.com')) stats.cloudinary++;
    else if (url.startsWith('/images/products/')) stats.localProducts++;
    else if (url.startsWith('/images/srp_') || url.startsWith('/images/SRP')) stats.localSrp++;
    else stats.other++;
  }
